            ...
            <section className="relative pt-20 pb-32 overflow-hidden">
                ...
                <div className="mx-auto px-4 text-center container">
                    ...
                    <div className="flex sm:flex-row flex-col justify-center items-center gap-5">
                        <Link to="/dashboard" className="flex items-center gap-2 bg-primary shadow-2xl shadow-primary/30 hover:shadow-primary/50 px-8 py-5 rounded-2xl focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background font-bold text-primary-foreground text-lg transition-all">
                            <Plus className="w-6 h-6" /> Start a Jam Session
                        </Link>
                        <Link to="/about" className="flex items-center gap-2 bg-card/50 hover:bg-card backdrop-blur-sm px-8 py-5 border border-border rounded-2xl focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:ring-offset-background font-bold text-lg transition-all">
                            <Book className="w-5 h-5 text-secondary" /> Read More About Us
                        </Link>
                    </div>
                </div>
            </section>

            <section id="features" className="relative bg-muted/30 py-32 overflow-hidden">
                    ...
                    <div className="gap-8 grid md:grid-cols-3">
                        {CORE_FEATURES.map((feature, i) => (
                            <motion.div
                                ...
                                className={`group relative bg-card/40 hover:bg-card/80 backdrop-blur-xl p-8 md:p-10 border border-border/50 ${feature.colorBorder} rounded-[2.5rem] transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl overflow-hidden`}
                            >
                                <div className={`absolute -top-12 -right-12 w-48 h-48 ${feature.colorGlow} rounded-full blur-[60px] opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none`} />

                                <div className={`font-mono text-[10px] mb-6 font-bold uppercase tracking-[0.2em] ${feature.colorText} opacity-80`}>
                                    [{feature.tag}]
                                </div>

                                <div className={`relative flex justify-center items-center ${feature.colorBg} mb-8 rounded-3xl w-16 h-16 transition-transform duration-500 group-hover:scale-110 group-hover:-rotate-6 shadow-inner`}>
                                    <div className={`absolute inset-0 border-2 border-current opacity-10 rounded-3xl ${feature.colorText}`} />
                                    <feature.icon className={`w-8 h-8 ${feature.colorText}`} />
                                </div>

                                <h3 className="mb-4 font-black text-2xl md:text-3xl uppercase tracking-tighter">
                                    {feature.title}
                                </h3>
                                ...
                                <div className={`absolute bottom-0 left-10 right-10 h-[3px] bg-linear-to-r from-transparent via-current to-transparent opacity-0 group-hover:opacity-20 ${feature.colorText} transition-opacity duration-500`} />
                            </motion.div>
                        ))}
                    </div>
                </div>
